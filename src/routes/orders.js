  ...
  try {
    ...
    if (status === 'confirmed') {
      const { data: ctx } = await supabase.from('orders')
        .select('id, final_price, design_thumbnail_url, bakers(name, slug, logo_url), customers(email, phone, first_name)')
    ...
    if (status === 'ready') {
      const { data: ctx } = await supabase.from('orders')
        .select('id, delivery_mode, delivery_date, delivery_time, design_thumbnail_url, bakers(name, slug, logo_url), customers(email, phone, first_name)')
        .eq('id', req.params.id).maybeSingle();
    ...
    if (status === 'completed') {
      const { data: ctx } = await supabase.from('orders')
        .select('id, design_thumbnail_url, bakers(name, slug, logo_url), customers(email, phone, first_name)')
        .eq('id', req.params.id).maybeSingle();
...
router.post('/orders/:id/quote', requireAuth, requireCapability('order:manage'), async (req, res) => {
  try {
    ...
    const existingRow = await assertBakerOwns(req, 'orders', req.params.id, { select: 'status_id, current_version_id, design_thumbnail_url, order_statuses ( key ), bakers(name, slug, logo_url), customers(email, phone, first_name)' });
    if (!existingRow) return res.status(404).json({ error: 'Order not found' });
    const existing = withStatusKey(existingRow);
...
router.patch('/orders/:id/design', requireAuth, requireCapability('order:manage'), async (req, res) => {
  try {
    ...
    const existingRow = await assertBakerOwns(req, 'orders', req.params.id, { select: 'id, status_id, order_statuses ( key ), bakers(name, slug, logo_url), customers(email, phone, first_name, last_name)' });
    if (!existingRow) return res.status(404).json({ error: 'Order not found' });
    const existing = withStatusKey(existingRow);
