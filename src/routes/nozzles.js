...
const FIELDS =
  ...
  + 'image_approved_at, image_approved_by, tip_teeth, tip_cut, is_common, sort_order, is_active, '
...
function buildPayload(body, { partial = false } = {}) {
  ...
  // The shape of the opening, for the drawing. NULL is meaningful and must survive: "we do not know
  // how many teeth this tip has" is a real answer, and it is the one that stops a drawing being
  // invented. So an empty value clears the column rather than coercing to 0 the way sort_order does.
  const setNum = (key, val, lo, hi) => {
    if (val === undefined) return;
    if (val === null || String(val).trim() === '') { out[key] = null; return; }
    const n = Number(val);
    out[key] = Number.isFinite(n) ? Math.min(hi, Math.max(lo, n)) : null;
  };
  setNum('tip_teeth', body.tip_teeth, 3, 64);
  setNum('tip_cut',   body.tip_cut,   0, 1);
