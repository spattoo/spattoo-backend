...
// ── The grid ─────────────────────────────────────────────────────────────────────────
// ONE PANEL PER STEP, and the grid's shape is DERIVED from the step count rather than stored.
//
// That is what lets each step show its own picture beside its own words. A grid over here and a
// list of instructions over there is two things to hold in your head at a bench; the reference
// sheets bakers actually share put the words in the panel. We cannot put text INSIDE the generated
// image (models misspell, and a mangled hex wastes a batch), so instead every step renders the one
// cell of the grid that belongs to it — background-position on screen, a source rect in the PDF.
// No slicing, no second asset, no extra call.
//
// Derived, not stored, so a reader can compute the same layout from the guide it already has —
// there is no row to migrate and nothing that can disagree with the image it describes.
//
// Capped at 12: past that the panels are too small to read anything in, and a guide needing more
// than a dozen steps is one that should have been written as fewer, meatier ones.
export const MAX_STAGES = 12;

export function stageGrid(stepCount) {
  const n = Math.min(MAX_STAGES, Math.max(1, Number(stepCount) || 1));
  // 3 across reads well at sheet width and keeps panels square-ish; 2 across for very short guides
  // so four steps do not become a thin strip.
  const cols = n <= 4 ? 2 : 3;
